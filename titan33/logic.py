...
# from main import check_and_make_directories  //从main里面导入，会有循环引用的问题。所以我直接复制过来一份。直接用了。
...
def start_train(stock_list, train_s, train_e, trade_s, trade_e):
  ticker_list = stock_list

  TRAIN_START_DATE = train_s
  TRAIN_END_DATE= train_e
  TRADE_START_DATE = trade_s
  TRADE_END_DATE = trade_e
  TIME_INTERVAL = "1d" 
  kwargs = {} 
  ...
  perf_stats_all_ddpg = perf_func(returns=daily_return)
  print("==============DDPG Strategy Stats===========")
  print(f"perf_stats_all: {perf_stats_all_ddpg}")
  ...
  perf_func = timeseries.perf_stats 
  perf_stats_all_a2c = perf_func(returns=daily_return)
  print("==============A2C Strategy Stats===========")
  print(f"perf_stats_all: {perf_stats_all_a2c}")

  return perf_stats_all_ddpg, perf_stats_all_a2c
